...
export const editCategory = (data) => {
  return async (dispatch) => {
    ...
    setAuthToken(token);
    API.defaults.headers.common['Content-Type'] = 'multipart/form-data';
    const res = await API.patch('/category/' + data.category_code, {'category_name' : data.category_name}).catch((err) => {
      return Promise.reject(err);
    });
    return res.data;
  };
...
export const addGamesList = (params) => {
  return async (dispatch) => {
    ...
    setAuthToken(token);
    API.defaults.headers.common["Content-Type"] = "multipart/form-data";
    const res = await API.post("/games", params).catch((err) => {
      return Promise.reject(err);
    });
...
export const editGamesList = (params) => {
  return async (dispatch) => {
    ...
    setAuthToken(token);
    API.defaults.headers.common["Content-Type"] = "multipart/form-data";
    const res = await API.patch("/games/" + params.id, params).catch((err) => {
      return Promise.reject(err);
    });
    return res.data;
  };
